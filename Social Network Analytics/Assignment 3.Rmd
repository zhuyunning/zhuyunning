...
# hand side of the dendrogramâ???"I canâ???Tt figure it out. If itâ???Ts not this, we should #
...
# modularity value from those 50. They call this the â???omaximum modularity 
# partitionâ??? and insert the parenthetical â???o(though, strictly speaking, this cannot be 
...
# Answer:  There is no "right" distance value nor number of clusters. One clustering
# may be better than another by one metric, and the reverse may be true using
# another metric.  Therefore, let us assume a distance value of 0.8.  Then the
# dendrogram indicates 3 clusters.
...
# Answer:  There is no "right" distance value nor number of clusters. One clustering
# may be better than another by one metric, and the reverse may be true using
# another metric.  Therefore, let us assume a distance value of 11.  Then the 
# dendrogram indicates 3 clusters.
...
# Answer:  A K-core is an undirected graph in which every subgraph has a vertex of degree
# at most k; a clique is an undirected graph in which every subgraph is complete.  K-cores
# are therefore more inclusive and flexible.  We can change the criterion that requires the 
# number of ties and dictate the numbers and sizes of the groups. 
